...
const streamAddressStashKey reconcilers.StashKey = "stream-address"
...
func StreamReconciler(c reconcilers.Config, provisioner StreamProvisionerClient) *reconcilers.ParentReconciler {
...
func StreamProvisionReconciler(c reconcilers.Config, provisioner StreamProvisionerClient) reconcilers.SubReconciler {
	...
	return &reconcilers.SyncReconciler{
		Sync: func(ctx context.Context, stream *streamingv1alpha1.Stream) error {
			...
			reconcilers.StashValue(ctx, streamAddressStashKey, *address)
...
func StreamChildBindingMetadataReconciler(c reconcilers.Config) reconcilers.SubReconciler {
	...
	return &reconcilers.ChildReconciler{
		...
		DesiredChild: func(ctx context.Context, parent *streamingv1alpha1.Stream) (*corev1.ConfigMap, error) {
			_, ok := reconcilers.RetrieveValue(ctx, streamAddressStashKey).(StreamAddress)
			if !ok {
				return nil, nil
			}
...
func StreamChildBindingSecretReconciler(c reconcilers.Config) reconcilers.SubReconciler {
	...
	return &reconcilers.ChildReconciler{
		...
		DesiredChild: func(ctx context.Context, parent *streamingv1alpha1.Stream) (*corev1.Secret, error) {
			address, ok := reconcilers.RetrieveValue(ctx, streamAddressStashKey).(StreamAddress)
...
func StreamSyncBindingCondition(c reconcilers.Config) reconcilers.SubReconciler {
