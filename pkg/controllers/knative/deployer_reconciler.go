...
func DeployerChildConfigurationReconciler(c reconcilers.Config) reconcilers.SubReconciler {
	...
	return &reconcilers.ChildReconciler{
		...
		DesiredChild: func(ctx context.Context, parent *knativev1alpha1.Deployer) (*servingv1.Configuration, error) {
			...
			template.Annotations = reconcilers.MergeMaps(annotations, template.Annotations)
			template.Labels = reconcilers.MergeMaps(labels, template.Labels)
...
func DeployerChildRouteReconciler(c reconcilers.Config) reconcilers.SubReconciler {
