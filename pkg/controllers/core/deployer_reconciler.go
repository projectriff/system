...
func DeployerReconciler(c reconcilers.Config) *reconcilers.ParentReconciler {
...
func DeployerBuildRefReconciler(c reconcilers.Config) reconcilers.SubReconciler {
...
func DeployerChildDeploymentReconciler(c reconcilers.Config) reconcilers.SubReconciler {
...
func DeployerChildServiceReconciler(c reconcilers.Config) reconcilers.SubReconciler {
	...
	return &reconcilers.ChildReconciler{
		...
		ReflectChildStatusOnParent: func(parent *corev1alpha1.Deployer, child *corev1.Service, err error) {
			...
			if child == nil {
				...
				parent.Status.Address = &duckv1.Addressable{URL: fmt.Sprintf("http://%s.%s.%s", child.Name, child.Namespace, "svc.cluster.local")}
...
func DeployerChildIngressReconciler(c reconcilers.Config) reconcilers.SubReconciler {
