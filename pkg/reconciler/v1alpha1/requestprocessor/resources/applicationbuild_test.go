...
func TestApplicationBuild(t *testing.T) {
	...
	rp.Spec = append(rp.Spec, runv1alpha1.RequestProcessorSpecItem{
		Build: &runv1alpha1.Build{
			Application: &buildv1alpha1.ApplicationBuildSpec{
	...
	ab, _ := MakeApplicationBuild(rp, 0)

	if errs := ab.Validate(context.Background()); errs != nil {
		t.Errorf("expected valid application build got errors %+v", errs)
	}
	...
	if diff := cmp.Diff(*rp.Spec[0].Build.Application, ab.Spec); diff != "" {
