...
func makeLabels(ab *buildv1alpha1.ApplicationBuild) map[string]string {
	labels := make(map[string]string, len(ab.ObjectMeta.Labels)+1)
	labels[build.ApplicationBuildLabelKey] = ab.Name

	// Pass through the labels on the ApplicationBuild to child resources.
