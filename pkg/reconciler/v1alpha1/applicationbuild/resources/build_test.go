...
func TestBuild(t *testing.T) {
	ab := createApplicationBuildMeta()
	ab.Labels = map[string]string{testLabelKey: testLabelValue}
	ab.Spec.Image = testImage
	ab.Spec.Source.Git = &buildv1alpha1.GitSource{
		URL:      testGitURL,
		Revision: testGitRevision,
	}
	ab.Status.BuildCacheName = testBuildCacheName
	...
	if got, want := b.Namespace, testApplicationBuildNamespace; got != want {
	...
	if got, want := b.Labels[build.ApplicationBuildLabelKey], testApplicationBuildName; got != want {
		t.Errorf("expected %q labels got %q", want, got)
	}
