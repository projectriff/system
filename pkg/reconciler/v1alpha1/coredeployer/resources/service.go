...
// MakeService creates a Service from a Deployer object.
func MakeService(d *corev1alpha1.Deployer) (*corev1.Service, error) {
	if d.Status.DeploymentName == "" {
		return nil, fmt.Errorf("unable to create Service, waiting for Deployment")
	}

	service := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      names.Service(d),
