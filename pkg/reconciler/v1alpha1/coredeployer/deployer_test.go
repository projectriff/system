...
func TestReconcile(t *testing.T) {
	...
	table.Test(t, MakeFactory(func(listers *Listers, opt reconciler.Options) controller.Reconciler {
		return &Reconciler{
			...
			deployerLister:    listers.GetCoreDeployerLister(),
...
func TestNew(t *testing.T) {
	...
	deployerInformer := projectriffInformer.Core().V1alpha1().Deployers()
	...
	c := NewController(reconciler.Options{
		...
		Logger:               TestLogger(t),
	}, deployerInformer, deploymentInformer, serviceInformer, applicationInformer, containerInformer, functionInformer)
