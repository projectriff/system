...
func TestBuildCache(t *testing.T) {
	...
	fb.Spec.CacheSize = &cacheSize

	c, _ := MakeBuildCache(fb)

	if got, want := c.Name, testBuildCacheName; got != want {
		t.Errorf("expected %q for build cache name got %q", want, got)
	}
	if got, want := c.Namespace, testFunctionBuildNamespace; got != want {
	...
	if got, want := c.Labels[build.FunctionBuildLabelKey], testFunctionBuildName; got != want {
		t.Errorf("expected %q labels got %q", want, got)
	}
