...
func NewController(
	...
	functionInformer.Informer().AddEventHandler(reconciler.Handler(impl.Enqueue))
...
func (c *Reconciler) reconcile(ctx context.Context, function *buildv1alpha1.Function) error {
	...
	if function.Status.GetCondition(buildv1alpha1.FunctionConditionBuildSucceeded).IsTrue() {
		...
		if err != nil {
			c.Recorder.Eventf(function, corev1.EventTypeNormal, "ImageMissing", "Unable to fetch image %q: %s", function.Status.TargetImage, err.Error())
			return err
		}
...
func (c *Reconciler) resolveTargetImage(function *buildv1alpha1.Function) (string, error) {
