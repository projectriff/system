...
func (d *Deployer) Validate(ctx context.Context) *apis.FieldError {
	...
	errs = errs.Also(systemapis.ValidateObjectMetadata(d.GetObjectMeta()).ViaField("metadata"))
	errs = errs.Also(d.Spec.Validate(ctx).ViaField("spec"))
	return errs
}

func (ds DeployerSpec) Validate(ctx context.Context) *apis.FieldError {
	if equality.Semantic.DeepEqual(ds, DeployerSpec{}) {
	...
	if diff := cmp.Diff(&corev1.PodSpec{
		...
		ServiceAccountName: ds.Template.ServiceAccountName,
		// the defaulter guarantees at least one container
		Containers: filterInvalidContainers(ds.Template.Containers[:1]),
		Volumes:    filterInvalidVolumes(ds.Template.Volumes),
	}, ds.Template); diff != "" {
	...
	if ds.Build == nil && ds.Template.Containers[0].Image == "" {
		errs = errs.Also(apis.ErrMissingOneOf("build", "template.containers[0].image"))
	} else if ds.Build != nil && ds.Template.Containers[0].Image != "" {
		errs = errs.Also(apis.ErrMultipleOneOf("build", "template.containers[0].image"))
	} else if ds.Build != nil {
		errs = errs.Also(ds.Build.Validate(ctx).ViaField("build"))
	}
