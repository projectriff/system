	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.KnativeV1alpha1().Handlers(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.KnativeV1alpha1().Handlers(namespace).Watch(options)
			},
		},
		&knative_v1alpha1.Handler{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *handlerInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&knative_v1alpha1.Handler{}, f.defaultInformer)
}
