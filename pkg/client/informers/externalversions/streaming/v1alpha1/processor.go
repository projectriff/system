	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.StreamingV1alpha1().Processors(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.StreamingV1alpha1().Processors(namespace).Watch(options)
			},
		},
		&streaming_v1alpha1.Processor{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *processorInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&streaming_v1alpha1.Processor{}, f.defaultInformer)
}
