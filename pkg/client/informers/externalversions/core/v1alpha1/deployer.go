	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.CoreV1alpha1().Deployers(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.CoreV1alpha1().Deployers(namespace).Watch(options)
			},
		},
		&core_v1alpha1.Deployer{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *deployerInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&core_v1alpha1.Deployer{}, f.defaultInformer)
}
