...
readonly SYSTEM_YAML=${YAML_OUTPUT_DIR}/riff-system.yaml

# Flags for all ko commands
readonly KO_YAML_FLAGS="-P ${KO_FLAGS}"

if [[ -n "${TAG}" ]]; then
  LABEL_YAML_CMD=(sed -e "s|projectriff.io/release: devel|projectriff.io/release: \"${TAG}\"|")
else
  LABEL_YAML_CMD=(cat)
fi
...
ko resolve ${KO_YAML_FLAGS} -f config/ | "${LABEL_YAML_CMD[@]}" > "${SYSTEM_YAML}"

echo "All manifests generated"

# List generated YAML files, with riff-system.yaml first.

ls -1 ${SYSTEM_YAML} > ${YAML_LIST_FILE}
ls -1 ${YAML_OUTPUT_DIR}/*.yaml | grep -v ${SYSTEM_YAML} >> ${YAML_LIST_FILE}
